...
Const ENTITY_OBJECT  = 4 ; Tower and Barrel
Const ENTITY_BEAST = 5
Const ENTITY_BULLET = 6
...
Function Setup()
	...
	bull_sprite = LoadSprite( "gfx/sparkred.jpg" )
...
Function CreateBullet.Bullet(obj)
	...
	EntityType b\sprite, ENTITY_BULLET
	Return b
End Function

Function UpdateBullet(b.Bullet)
	If EntityCollided(b\sprite, ENTITY_BEAST)
;	If EntityCollided(b\sprite, ENTITY_TERRAIN)
;	If EntityCollided(b\sprite, ENTITY_PLANE)
;	If EntityCollided(b\sprite, ENTITY_OBJECT)
	...
	;DETECT COLLISIONS:
	item = EntityCollided(b\sprite, ENTITY_BEAST)	
	If EntityY(b\sprite) > 800
			FreeEntity b\sprite
		Delete b
 	EndIf 
End Function
...
Function CreateSpark.Spark(b.Bullet)
  If EntityCollided(ENTITY_BULLET, ENTITY_BEAST) Then
	s.Spark=New Spark
	spark = LoadMesh("gfx/bigspark.bmp")
	EndIf
	s\alpha=-90
...
Function GameOver()
	If Health=0 Then
	GameOver = LoadTexture("asesueldudievas.jpg")
EndIf 
