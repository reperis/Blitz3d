...
Function LoadDogs(file$)
	model1=LoadAnimMesh(file$)
	ExtractAnimSeq model1,2,14
	ScaleEntity model1,2,2,2
	For c=1 To 50
		Pack.dog = New dog
		Pack\ID = CopyEntity(model1)
		x=Rand(2000,2500):z=Rand(2000,2500)
		PositionEntity Pack\ID, x,150,z
		Animate Pack\ID,1,.3,1
		EntityAutoFade Pack\ID,500,1000
	EntityType Pack\ID, ENTITY_DOG
		EntityRadius Pack\ID,5
		zard = LoadTexture("zombiecolors.jpg") 
		EntityTexture (model1, zard)
		LoadAnimSeq(model1, "ZARD_biped walk.max") 
	Next
	FreeEntity model1
End Function
Function LoadSkull(gaidys$)
model1=LoadAnimMesh(gaidys$)
	;ExtractAnimSeq model1,2,14
	ScaleEntity model1,2,2,2
	For c=1 To 40
		Pack.dog = New dog
		Pack\ID = CopyEntity(model1)
		x=Rand(200,250):z=Rand(200,250)
		PositionEntity Pack\ID, x,150,z
		Animate Pack\ID,1,.3,1
		EntityAutoFade Pack\ID,500,1000
	EntityType Pack\ID, ENTITY_DOG
...
Function MoveDogs(obj)
	For moveall.dog = Each dog
		If Rand(1,100)=100 Then PointEntity moveall\ID,obj Else TurnEntity moveall\ID,0,Rand(-2,2),0
		MoveEntity moveall\ID,0,0,.5
		ex#=EntityX(moveall\ID):ez#=EntityZ(moveall\ID)
		PositionEntity moveall\ID,ex,150,ez
	Next
End Function
...
Collisions ENTITY_DOG, ENTITY_DOG,1,2
Collisions ENTITY_DOG, ENTITY_CAM,1,2
Collisions ENTITY_CAM, ENTITY_DOG,1,2
...
If KeyHit(57)
		trans=10-trans
If AnimSeq(model1)=0 Animate model1,  1,.5,1,trans
	Else
		If AnimSeq(model1)=1 Animate model1,2,1,0,trans
